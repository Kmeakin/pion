...
impl SyntaxTree {
    ...
    pub(crate) fn from_postorder(text: String, events: &[ParseEvent]) -> Self {
    ...
    pub fn left_sibling(&self) -> Option<SyntaxElement<'tree>> {
        ...
        match self.tree.data.get(self.self_index as usize - 1) {
            ...
            Some(TreeDatum::Node {
                ...
                debug_assert_eq!(*parent_index, self.self_index);
...
impl<'tree> SyntaxNode<'tree> {
    ...
    pub fn ancestors_exclusive(&self) -> impl Iterator<Item = SyntaxNode<'tree>> {
